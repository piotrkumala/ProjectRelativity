...
public class PlaceOrder
{
        ...
        HttpRequest req, ILogger log)
    {
        ...
        var (userId, orderItems) =
            JsonConvert.DeserializeObject<Order>(await new StreamReader(req.Body).ReadToEndAsync());
        if (orderItems.Count == 0)
        {
            return new UnprocessableEntityObjectResult("Cannot place order without items in it");
        }

        var entity = new DB.Entities.Order
        {
            UserId = userId, OrderItems = orderItems.Select(x => new OrderItem
            {
                Amount = x.Amount, Item = _dbContext.Items.FirstOrDefault(item => item.Id == x.Item.Id), ItemId = x.Item.Id
            }).ToList()
        };
